...
// movd
// vld1q_dup_s32(src)
#define load_dword(dst, src) { dst = vdupq_n_s8(0); ((int32_t *) &dst)[0] = src; }
...
#define movq(dst, src) dst = vextq_s8(vextq_s8(vdupq_n_s8(0), vld1q_s8(src), 8), vdupq_n_s8(0), 8)
// vld1q_s16(src)

// FIXME: vld1q하고 vld1q_dup를 혼용해서 사용해야 함
// #define movdqa(dst, src) dst = vld1q_dup_s16(src) // vst1q_s16을 써야 하는게 아닌지?
#define movdqa(dst, src) vst1q_u8((void *) dst, src)
...
#define pslldq(vec, amount) vec = vextq_s8(vdupq_n_s8(0), vec, 16 - amount)
#define psrldq(vec, amount) vec = vextq_s8(vec, vdupq_n_s8(0), amount)
